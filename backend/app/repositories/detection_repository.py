...
def fetch_detection_history(
    ...
    """
    Read stored classified detections from PostGIS.
    ...
    query = text(
        ...
        WHERE
            ...
            AND (
                CAST(:classification AS VARCHAR) IS NULL
                OR final_classification =
                    CAST(:classification AS VARCHAR)
            )
            AND ST_Intersects(
                geom,
                ST_MakeEnvelope(
    ...
    for row in rows:
        ...
        if acquisition_utc is not None:
            item["acquisition_utc"] = (
