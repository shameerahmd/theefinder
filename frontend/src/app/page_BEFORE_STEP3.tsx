...
export default function Home() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setFailedCount(payload.failed_count ?? payload.failures?.length ?? 0);
      ...
      if (normalized.length === 0) {
        ...
        const industrialIndex = normalized.findIndex(
          (item) =>
            item.final_classification === "POTENTIAL_INDUSTRIAL_FIRE" ||
            item.final_classification === "PERSISTENT_INDUSTRIAL_SOURCE",
        );

        setSelectedIndex(industrialIndex >= 0 ? industrialIndex : 0);
      }
    } catch (err) {
      const message =
